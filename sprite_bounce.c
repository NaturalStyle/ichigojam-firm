...
void __not_in_flash("render") render_loop() {
    ...
    while (1) {
        ...
        for (uint y = 0; y < FRAME_HEIGHT; ++y) {
            ...
            // sprite_blit16(pixbuf, (const uint16_t *)testcard_320x240 + (y +
            // frame_ctr / 2) % 240 * FRAME_WIDTH, 320);
            sprite_fill16(pixbuf, 0x07ff, FRAME_WIDTH);
            for (int i = 0; i < N_BERRIES; ++i)
                // sprite_asprite16(pixbuf, &berry[i], atrans[i], y,
                // FRAME_WIDTH);
...
int main() {
    ...
    // Slow everything down uniformly, so signals are probeable but the code
    // runs identically (note this actually uses the PLL with low feedback and
    // max PD1/PD2)
    ...
    // Core 1 will fire up the DVI once it sees the first colour buffer has been
    // rendered
