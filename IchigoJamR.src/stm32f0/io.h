...
void io_init() {
	// BTN
	GPIOA->MODER = (GPIOA->MODER & ~(3 << (2 * 0))) | (GPIO_MODER_IN << (2 * 0)); // PA0
	GPIOA->PUPDR |= GPIO_PUPDR_PUP << (2 * 0); // PA0
	
	for (int i = 1; i <= 8; i++) {
		GPIOA->MODER = (GPIOA->MODER & ~(3 << (2 * i))) | (GPIO_MODER_IN << (2 * i));
		GPIOA->PUPDR |= GPIO_PUPDR_PDOWN << (2 * i);
	}
	for (int i = 12; i <= 15; i++) {
		GPIOA->MODER = (GPIOA->MODER & ~(3 << (2 * i))) | (GPIO_MODER_IN << (2 * i));
		GPIOA->PUPDR |= GPIO_PUPDR_PDOWN << (2 * i);
	...
	for (int i = 1; i <= 7; i++) {
		if (i >= 2 && i <= 5)
			continue;
		GPIOB->MODER = (GPIOB->MODER & ~(3 << (2 * i))) | (GPIO_MODER_IN << (2 * i));
		GPIOB->PUPDR |= GPIO_PUPDR_PDOWN << (2 * i);
	}
	
	// LED
	GPIOA->MODER = (GPIOA->MODER & ~(3 << (2 * 12))) | (1 << (2 * 12)); // PA12 as output
}
// ADC
INLINE int analog_get(int ch) {
	return 0;
...
INLINE void IJB_led(int st) {
	if (st) {
		GPIOA->ODR |= 1 << 12; // PA12 on
	} else {
		GPIOA->ODR &= ~(1 << 12); // PA12 off
	}
//	IJB_out(7, st != 0);
}
INLINE int IJB_in() {
