...
//	int t1, t2, t3;
	for (timeout = 0; !LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]; timeout++) {
...
//	for (int i = 0; i < 10; i++) video_waitSync(); // 100usec wait
	...
	LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA] = 0; // start bit = 0
...
//	t3 = timeout; // 最大で7200くらい
	...
	for (int i = 0; i < 8; i++) {
		int bit = data & (1 << i);
		LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA] = bit ? ~0 : 0;
		if (bit)
			cnt++;
		while (!LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
		while (LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
	}
	LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA] = cnt & 1 ? 0 : ~0; // parity
	while (!LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
	while (LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
	
	LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA] = ~0; // stop bit = 1
	while (!LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
	while (LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]);
	
	LPC_GPIO0->DIR &= ~(1 << PS2KB_DATA);
	while (!LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA]); // wait data
	while (!LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]); // wait clk
	while (LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_DATA]); // wait data
	while (LPC_GPIO0->MASKED_ACCESS[1 << PS2KB_CLK]); // wait clk
