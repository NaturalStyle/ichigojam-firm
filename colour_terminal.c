...
void core1_main() {
    ...
    for (int i = 0; i < 10000; i++) {
        gpio_put(led, (i / 100) % 2 == 0);
        for (uint y = 0; y < FRAME_HEIGHT / 1; ++y) {
            uint32_t* tmdsbuf;
            ...
            for (int plane = 0; plane < 3; ++plane) {
                tmds_encode_font_2bpp(
                    ...
                    FRAME_WIDTH,
                    (const uint8_t*)&font_8x8[(y + (i / 1000) * 8) % FONT_CHAR_HEIGHT * FONT_N_CHARS] - FONT_FIRST_ASCII
                );
            }
...
int __not_in_flash("main") main() {
    ...
    for (uint y = 0; y < CHAR_ROWS; ++y) {
        for (uint x = 0; x < CHAR_COLS; ++x) {
            ...
            //     for (uint x = 0; x < 10; ++x) {
            set_char(x, y,
                (x + y * CHAR_COLS) % FONT_N_CHARS + FONT_FIRST_ASCII);
            set_colour(x, y, 0xFF, 0x00);
        }
    }
