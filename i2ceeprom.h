...
4~7ビット目は1010で固定*/
//24FC1025-I/Pの容量は1Mbit=128KB=2^17Bなので、client addressの3ビット目とその後に送る2バイトの情報でアドレスを決めるらしい
...
static uint8_t calc_client_address(int address) {
...
void make_write_src(int address, uint8_t* list, int len, uint8_t* src) {
...
// err:1 no_err:0
int write_eeprom(int address, uint8_t* list, int len) {
    ...
    while (len > 0) {
        ...
        len -= tlen;
        make_write_src(address, list, tlen, src);
        address += tlen;
        list += tlen;
