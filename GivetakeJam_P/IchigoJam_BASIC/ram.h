...
// [OLD] (1024 + 2)
// [NEW] (4096 + 2)
#define SIZE_RAM_LIST (4096 + 2) // 0x1002
...
#define SIZE_RAM (SIZE_RAM_PCG + SIZE_RAM_VRAM + SIZE_RAM_VAR + SIZE_RAM_VAR2 + SIZE_RAM_LIST + SIZE_RAM_KEYBUF + SIZE_RAM_LINEBUF + SIZE_RAM_I2CBUF)
...
// This prevents memcpy(list, ..., 4096) overflow when loading from flash.
// [OLD] total SIZE_RAM was too small (e.g., 2688 bytes), leaving only ~1408 bytes after OFFSET_RAM_LIST.
// [NEW] total SIZE_RAM is derived from last offset + last buffer size.
#undef SIZE_RAM
#define SIZE_RAM (SIZE_RAM_PCG + SIZE_RAM_VRAM + SIZE_RAM_VAR + SIZE_RAM_VAR2 + SIZE_RAM_LIST + SIZE_RAM_KEYBUF + SIZE_RAM_LINEBUF + SIZE_RAM_I2CBUF)
